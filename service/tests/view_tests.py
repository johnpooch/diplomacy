...
class TestEditOrder(APITestCase):
    ...
    def setUp(self):
        ...
        self.territory_state = factories.TerritoryStateFactory(
            turn=self.turn,
            controlled_by=self.nation_state.nation,
        )
        ...
        self.url = reverse(
            'update-order',
            args=[self.game.id, self.order.id],
        )
    ...
    def test_update_source(self):
        ...
        self.assertEqual(self.order.source, territory)
    ...
    def test_update_order_type(self):
        ...
        self.order.refresh_from_db()
        self.assertEqual(self.order.source, self.territory)
        self.assertEqual(self.order.target, territory)
        self.assertEqual(self.order.type, OrderType.MOVE)

    def test_update_other_players_order(self):
        other_user = factories.UserFactory()
        self.game.participants.add(other_user)
        self.client.force_authenticate(user=other_user)
        factories.NationStateFactory(
            turn=self.turn,
            user=other_user,
        )
        territory_state = factories.TerritoryStateFactory(
        ...
        self.order.refresh_from_db()
        self.assertEqual(self.order.source, self.territory)
        self.assertEqual(self.order.target, None)
        self.assertEqual(self.order.type, OrderType.HOLD)
