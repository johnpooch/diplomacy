"""
This script contains the territory class and subclasses. Special coastal
and special inland territories represent the three territories which have two
separate coasts. Special coasta inheritx from the Water class becasue they
...
class InlandTerritory(Territory):
    def __init__(self, name):
        Territory.__init__(self, name)

    def accessible_by_piece_type(self, piece):
        return isinstance(piece, Army)


class CoastalTerritory(Territory):
    def __init__(self, name, identified_coasts=[]):
        Territory.__init__(self, name)
        self.identified_coasts = identified_coasts
