"""
Mixin classes for ``Order`` which provide logic for determining whether a
order is legal.
"""
from django.utils.translation import gettext as _

from core.exceptions import IllegalOrderException
...
class ChecksMixin:
    ...
    def check_illegal(self):
        ...
        try:
            ...
            if self.type == OrderType.BUILD:
                self.check_build_illegal()

        except IllegalOrderException as e:
    ...
    def _specifies_target_named_coast_if_fleet(self):
        """
        If the order piece is a fleet and the ``target`` is has named coasts,
        the order must specify a ``target_coast`` unless only one of the
        named coasts is within reach of the order piece, in which case the
    ...
    def _aux_piece_move_legal(self):
        if self.aux.piece.order.is_move:
            self.aux.piece.order.check_illegal()
            if self.aux.piece.order.illegal:
                raise IllegalOrderException(_(
                    'Illegal because the order of '
