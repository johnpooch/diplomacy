...
class Turn(models.Model):
    ...
    def toggle_surrender(self, user):
        """
        Toggle the user's surrender state.
        ...
        Raises:
            * `NationState.DoesNotExist` if the user is not controlling a
              nation during this turn.
        """
        self._ensure_current_turn()
        surrendering = self.surrenders.filter(
            status=SurrenderStatus.PENDING,
            user=user,
        )
        if surrendering:
            return self.cancel_surrender(user)
        return self.surrender_user(user)

    def surrender_user(self, user):
        """
        A user has decided to surrender. Create a `Surrender` instance, remove
        all user nation's orders.
        ...
        """
        self._ensure_current_turn()
        nation_state = self.nationstates.get(user=user)
        if self.surrenders.filter(
            user=user, status=SurrenderStatus.PENDING
        ).exists():
            raise ValueError('User is already surrendering')
        self.orders.filter(nation=nation_state.nation).delete()
        return self.surrenders.create(user=user)

    def cancel_surrender(self, user):
        """
        A user decided not to surrender.
        ...
        Raises:
            * `Surrender.DoesNotExist` if the user does not have a pending
              surrender.
        """
        self._ensure_current_turn()
        surrender = self.surrenders.get(
            user=user,
            status=SurrenderStatus.PENDING
        )
    ...
    @transaction.atomic
    def replace_surrendering_user(self, surrendering_user, new_user):
        """
        A new user is replacing a surrendering user.
        ...
        Raises:
            * `NationState.DoesNotExist` if the `surrendering_user` is not
              controlling a nation in the current turn.
            * `Surrender.DoesNotExist` if the `surrendering_user` does not have
              a pending surrender.
        """
        self._ensure_current_turn()
        nation_state = self.nationstates.get(user=surrendering_user)
        surrender = self.surrenders.get(
            user=surrendering_user,
            status=SurrenderStatus.PENDING
        )
        if new_user in self.game.participants.all():
            raise ValueError('New user is already a participant')
        surrender.status = SurrenderStatus.FULFILLED
        surrender.resolved_at = timezone.now()
        surrender.replaced_by = new_user
        surrender.save()
        nation_state.user = new_user
        nation_state.save()
        self.game.participants.add(new_user)
        return surrender, nation_state
